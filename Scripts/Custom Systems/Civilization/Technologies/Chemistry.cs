using System;
using Server;

namespace Server.UOC.Technologies
{
    public sealed class Chemistry
    {
        public static void Initialize()
        {
            TechType m_TechType = TechType.Chemistry;
            LokaiSkillName[] m_lSkillsAllowed = new LokaiSkillName[] { LokaiSkillName.Brewing };
            SkillName[] m_SkillsAllowed = new SkillName[] { SkillName.Forensics, SkillName.Poisoning, SkillName.Alchemy };
            GovernmentType m_GovernmentRequired = GovernmentType.Oligarchy;
            TechType[] m_TechsRequired = new TechType[] { TechType.Firemaking };
            string m_Description = "Chemistry temporary description.";
            string m_Name = "Chemistry";
            int m_Cost = 320000;
            int m_CorruptionFactor = -1;
            int m_WasteFactor = -4;
            int m_SatisfactionFactor = 0;
            int m_ProductionFactor = 2;
            int m_TradeFactor = 2;
            int m_TechnologyFactor = 5;

            UOC.Technology.Register(new Technology(m_TechType, m_SkillsAllowed, m_GovernmentRequired,
                m_TechsRequired, m_Description, m_Name, m_Cost, m_CorruptionFactor, m_WasteFactor,
                m_SatisfactionFactor, m_ProductionFactor, m_TradeFactor, m_TechnologyFactor));
        }
    }
}




