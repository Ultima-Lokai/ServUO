using System;
using Server;

namespace Server.UOC.Technologies
{
    public sealed class Currency
    {
        public static void Initialize()
        {
            TechType m_TechType = TechType.Currency;
            LokaiSkillName[] m_lSkillsAllowed = new LokaiSkillName[] { LokaiSkillName.Commerce };
            SkillName[] m_SkillsAllowed = new SkillName[] { SkillName.EvalInt };
            GovernmentType m_GovernmentRequired = GovernmentType.Oligarchy;
            TechType[] m_TechsRequired = new TechType[] { TechType.Alphabet };
            string m_Description = "Currency temporary description.";
            string m_Name = "Currency";
            int m_Cost = 160000;
            int m_CorruptionFactor = -2;
            int m_WasteFactor = -1;
            int m_SatisfactionFactor = 1;
            int m_ProductionFactor = 1;
            int m_TradeFactor = 4;
            int m_TechnologyFactor = 1;

            UOC.Technology.Register(new Technology(m_TechType, m_SkillsAllowed, m_lSkillsAllowed, m_GovernmentRequired,
                m_TechsRequired, m_Description, m_Name, m_Cost, m_CorruptionFactor, m_WasteFactor,
                m_SatisfactionFactor, m_ProductionFactor, m_TradeFactor, m_TechnologyFactor));
        }
    }
}




