...
namespace Server.ACC.PG
{
	public class PGGump : Gump
	{
		...
		{
			if( m_Page < 0 || m_Page >= PGSystem.Entries.Count )
				return -1;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			else if( BID == 2 && from.AccessLevel >= AccessLevel.GameMaster)
			{
			...
			else if( BID == 3 && from.AccessLevel >= AccessLevel.GameMaster )
			{
			...
			else if( BID == 4 && from.AccessLevel >= AccessLevel.GameMaster )
			{
				SetFlag( Conditions.Adding, true );
			...
			else if( BID == 5 && from.AccessLevel >= AccessLevel.GameMaster )
			{
				if( Loc <= -1 )
				{
					from.SendMessage( "You must select a location!" );
			...
			else if( BID == 6 && from.AccessLevel >= AccessLevel.GameMaster )
			{
				...
				((PGCategory)PGSystem.Entries[m_Page]).Locations.Add( PGL );
			}
