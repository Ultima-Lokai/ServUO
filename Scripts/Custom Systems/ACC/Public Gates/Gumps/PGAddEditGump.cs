			...
			if( BID == 1 )
			{
				if( GetFlag( Conditions.Category ) )
				{
					if( GetFlag( Conditions.Adding ) )
					{
						...
						{
							c = Int32.Parse( CS );
							PGSystem.Entries.Add( new PGCategory( NS, Flags, c ) );
					...
					else
					{
						...
						else
						{
							...
							((PGCategory)PGSystem.Entries[m_CurCat]).Name = NS;
							((PGCategory)PGSystem.Entries[m_CurCat]).Flags = Flags;
							if( CS == null || CS.Length == 0 )
								((PGCategory)PGSystem.Entries[m_CurCat]).Cost = 0;
							else
							{
								int c = 0;
								...
								{
									c = Int32.Parse( CS );
									((PGCategory)PGSystem.Entries[m_CurCat]).Cost = c;
								}
								catch
								{
									((PGCategory)PGSystem.Entries[m_CurCat]).Cost = 0;
